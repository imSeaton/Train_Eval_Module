...
def cross_validation_with_val_set(args, dataset, device, logger=None):
    ...
    assert epoch_select in ['val_min', 'test_max']
    ...
    for fold, (train_idx, test_idx, val_idx) in enumerate(
        ...
        for epoch in range(1, epochs + 1):
            ...
            val_loss, _ = eval_loss(args, model, val_loader, device, with_eval_mode)
    ...
    print('Train Acc: {:.2f}, Test Acc: {:.2f} {:.2f}, Test Acc (C*): {:.2f} {:.2f}, Duration: {:.3f}'.
          format(train_acc_mean * 100, test_acc_mean * 100, test_acc_std * 100, test_acc_epoch_mean * 100,
                 test_acc_epoch_std * 100, duration_mean))
    sys.stdout.flush()
...
def k_fold(dataset, folds, epoch_select):
    ...
    if epoch_select == 'test_max':
        # in this situation, val_indices = test_indices
...
def train(args, model, optimizer, loader, device, epoch, writer):
    ...
    for batch_iter, data in enumerate(loader):
        ...
        # 根据mask_batch中的True和False标签，创建batch，batch中指示了每个graph中的节点属于那个graph
        batch = torch.tensor([i for i in range(mask_batch.size(0)) for t in range(torch.sum(mask_batch[i]))]).to(device)
        # out.shape: (batch, num_targets)
        # reconstruction_loss.shape: (1, )
        ...
        loss.backward()
        # num_graph: 128
        # num_graphs是一个dataloader中有多少graph
        total_loss += loss.item() * num_graphs(data)

        writer.add_scalar('CE loss', loss_ce, (epoch - 1) * len(loader) + (batch_iter + 1))
        writer.add_scalar('Reconstruction loss', args.theta * reconstruction_loss,
                          (epoch - 1) * len(loader) + (batch_iter + 1))
        optimizer.step()
    # total_loss / len(loader.dataset): 所有graph上的平均损失
    # correct / len(loader.dataset): 训练精度
...
def eval_acc(model, loader, device, with_eval_mode):
    ...
    for data in loader:
        ...
        with torch.no_grad():
            features_batch, adj_batch, mask_batch = data.x, data.adj, data.mask
            batch = torch.tensor([i for i in range(mask_batch.size(0)) for t in range(torch.sum(mask_batch[i]))]).to(
                device)
...
def eval_loss(args, model, loader, device, with_eval_mode):
    if with_eval_mode:
        model.eval()
